...
class Cors
{
    ...
    public static function setSessionToken()
    {
        try {
            if (PHP_SESSION_ACTIVE !== session_status()) Session::init(config('session.'));
            if ($token = request()->header('AccessToken', input('AccessToken', ''))) {
    ...
    public static function optionsHandler()
    {
        ...
        if (request()->isOptions()) {
            ...
            header('Access-Control-Expose-Headers:Access-Session-Token');
    ...
    public static function getRequestHeader()
    {
        return [
            ...
            'Access-Control-Expose-Headers'    => 'Access-Session-Token',
            'Access-Session-Token'             => self::getSessionToken(),
        ];
    }
